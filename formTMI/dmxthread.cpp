...
void DMXThread::run()
{
    ...
    if (countParam.joinFramesSign)
    {
        ...
        for (int i = 0; i < frsavers.length(); i++) // Цикл по файлам
        {
            ...
            frsavers.at(i)->frames.at(0)->timeFrameS = inFreaders.at(i)->fBegTime.addMSecs(int(curTime));
            ...
            for (int j = 1; j < frsavers.at(i)->frames.length(); j ++) // Цикл по кадрам
            {
                ...
                if (!countInDetect || countOutDetect) // Если плохое начало или плохой конец
                {
                    ...
                    frsavers.at(i)->frames.at(j)->timeFrameS =
                            frsavers.at(i)->frames.at(0)->timeFrameS.addMSecs(int(curTime));
                }
                else
                {
        ...
        QTime GTSBegTime = outFrameSaver->frames.at(0)->timeFrameS;
        double timeLine = 0;
        ...
        qint64 firstTime = outFrameSaver->frames.at(0)->timeFrameS.hour() * 60 * 60 * 1000 +
                outFrameSaver->frames.at(0)->timeFrameS.minute() * 60 * 1000 +
                outFrameSaver->frames.at(0)->timeFrameS.second() * 1000 +
                outFrameSaver->frames.at(0)->timeFrameS.msec();
