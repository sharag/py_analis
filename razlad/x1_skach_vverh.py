...
len_win, len_win_bef, probabl_x2_ps_obr = max_probabil(surge_lin_ps, win_size, surge_len, step_win)
max_ind = list()
...
prob = f_probability(surge_lin_ps, win_bef, win_aft)
plt.figure(51)
plt.subplot(1, 1, 1)
x_axis = np.linspace(win_bef, len(surge_lin_ps) - win_aft, len(surge_lin_ps) - win_bef - win_aft)
...
for p_osh in p_osh_array:
    ...
    num_po_, num_lt_, num_pc_ = test_probability(num_test, surge_lin_ps, win_bef, win_aft, porog, indexes_skach, p_osh)
