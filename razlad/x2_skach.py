...
from razlad.functions import max_probabil, add_nois, f_probability, test_probability
...
num_test = 1000
# Скачок
surge_x2_ps_obr = [0.0]*(graph_len//2 - surge_len)
...
surge_x2_ps_obr.extend([0.0]*(graph_len//2 - surge_len))
# График скачка
signal = surge_x2_ps_obr
plt.figure(1)
plt.subplot(1, 1, 1)
...
len_win, len_win_bef, probabl_x2_ps_obr = max_probabil(surge_x2_ps_obr, win_size, surge_len, step_win)
max_ind = list()
...
prob = f_probability(surge_x2_ps_obr, win_bef, win_aft)
plt.figure(2)
plt.subplot(1, 1, 1)
x_axis = np.linspace(win_bef, len(surge_x2_ps_obr) - win_aft, len(surge_x2_ps_obr) - win_bef - win_aft)
...
for p_osh in p_osh_array:
    ...
    num_po_, num_lt_, num_pc_ = test_probability(num_test, surge_x2_ps_obr, win_bef, win_aft, porog, indexes_skach, p_osh)
